...
add_thickbox();
$the100_recommended_plugins = array(
	...
	<div class="feature-section recommended-plugins">
		<?php foreach ( $the100_recommended_plugins as $plugin => $prop ) { ?>
		<?php
		...
		switch ( $active['needs'] ) {
			...
			$label = __( 'Install', 'the100' );
			break;
			case 'activate':
			$class = 'activate-now button button-primary';
			$label = __( 'Activate', 'the100' );
			break;
			case 'deactivate':
			$class = 'deactivate-now button';
			$label = __( 'Deactivate', 'the100' );
			break;
		}
		...
		<div class="col plugin_box">
			<?php if ( $prop['recommended'] ): ?>
				<span class="recommended"><?php _e( 'Required', 'the100' ); ?></span>
			<?php endif; ?>
			...
			<div class="b-wrap">
			<span class="version"><?php echo __( 'Version:', 'the100' ); ?><?php echo esc_html( $info->version ) ?></span>
