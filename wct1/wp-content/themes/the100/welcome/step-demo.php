...
<div class="warning-msg"><?php echo __('Please install Recommended Plugins for demo to be imported completely.<br /> And Make sure you download demo in a fresh install. <br /> Your Old Data might be deleted.','the100');?></div>
<?php
...
foreach($the100_req_plugins as $slug=>$plugin) :
	if($plugin['bundled'] == false) {
		...
		<div class="action-tab warning">
			<h3><?php printf( esc_html__("Install : %s Plugin", 'the100'), $plugin['name'] ); ?></h3>
			<p><?php echo __('Please check the plugins folder inside theme and upload the zip of plugins from plugin uploader.','the100');?></p>
		</div>
		<?php
	} else {
