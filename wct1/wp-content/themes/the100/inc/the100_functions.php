...
function the100_featured_section_cb(){
				...
				if($the100_feat_cat>0){
					...
					if($feat->have_posts()){
						...
						while($feat->have_posts()){
							...
							the_title( '<h3 class="feat-title">', '</h3>' );
						...
						if($the100_service_cat>0){
							...
							if($service->have_posts()){
								...
								while($service->have_posts()){
									$service-> the_post();
									$j = $i/2;
									echo "<div class='service-posts wow fadeInDown' data-wow-delay='".$j."s'>";
									if($the100_aboutservice_layout=='lay-one'){
										if(has_post_thumbnail()){
											the_post_thumbnail('the100-square');
										}
									...
									echo "<div class='service-titledesc-wrap ".$class."'>";
									if($the100_aboutservice_layout=='lay-one'){
	...
	function the100_counter_section_cb(){
		?> <p class="title-aboutus"> About us </p> <?php

		$the100_ena = get_theme_mod('the100_home_counter','yes');
		if($the100_ena=='yes'){
			$bkg = get_theme_mod('the100_home_counter_bkg','');
			?>
			<section class="home-counter-section <?php if($bkg!=''){echo "has-image";}?>" <?php if($bkg!=''){echo "style='background-image:url(".$bkg.")'";}?>>
				<?php
			...
			if($the100_blog_cat>0){
				...
				if($blog->have_posts()){
					...
					while($blog->have_posts()){
						$blog-> the_post();
						$j = $i/2;
						echo "<div class='blog-posts wow fadeInDown' data-wow-delay='".$j."s'>";
						if($the100_blog_layout=='lay-two' || $the100_blog_layout=='lay-three'){
							...
							if($the100_blog_layout=='lay-two'){
								...
								?><p><?php $content = get_the_content(); $content = strip_tags($content); echo substr($content, 0, 170);?>...</p><?php
								echo "</div>";
							}
							echo "</div>";
						}else{
							...
							if($the100_blog_layout=='lay-one'){
								...
								echo '<span class="blog-comment">'.$comments.' '.__('Comments','the100').'</span>';
							}else{
