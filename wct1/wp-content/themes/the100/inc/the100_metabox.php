...
{ 
	...
	<table class="form-table">
		<tr>
			<td colspan="4"><em class="f13"><?php _e('Choose Sidebar Template','the100'); ?></em></td>
		</tr>
				...
				foreach ($the100_sidebar_layout as $field) {  
					...
					<div class="radio-image-wrapper" style="float:left; margin-right:30px;">
						<label class="description">
							...
							<input type="radio" name="the100_sidebar_layout" value="<?php echo $field['value']; ?>" <?php checked( $field['value'], $the100_sidebar_metalayout ); if(empty($the100_sidebar_metalayout) && $field['value']=='right-sidebar'){ echo "checked='checked'";} ?>/>&nbsp;<?php echo $field['label']; ?>
						</label>
					</div>
                <?php } // end foreach 
                ?>
...
function the100_save_sidebar_layout( $post_id ) { 
	...
	foreach ($the100_sidebar_layout as $field) {  
		...
		$new = sanitize_text_field($_POST['the100_sidebar_layout']);
		if ($new && $new != $old) {  
