...
function the100_widgets_show_widget_field($instance = '', $widget_field = '', $athm_field_value = '') {
    ...
    switch ($the100_widgets_field_type) {
        ...
        <p>
            ...
            $output .= '<label for="'.$id.'">'.$the100_widgets_title.'</label><br/>';
            $output .= '<input id="' . $id . '" class="upload' . $class . '" type="text" name="' . $name . '" value="' . $value . '" placeholder="' . __('No file chosen', 'the100') . '" />' . "\n";
            if (function_exists('wp_enqueue_media')) {
                if (( $value == '')) {
                    $output .= '<input id="upload-' . $id . '" class="upload-button-wdgt button" type="button" value="' . __('Upload', 'the100') . '" />' . "\n";
                } else {
                    $output .= '<input id="remove-' . $id . '" class="remove-file button" type="button" value="' . __('Remove', 'the100') . '" />' . "\n";
                }
            } else {
